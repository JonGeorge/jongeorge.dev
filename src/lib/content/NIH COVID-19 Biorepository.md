...
There was no way to catalog all specimens reliably, as this event exceeded the limitations of a shared Excel spreadsheet.

There was no way to share statistics and analytics on specimen data and associated studies with the NIH.

Getting accurate reports for leadership was an insurmountable task because the data was unstructured and disparate.

#### Solution
Provide a centralized system to view the specimen data, communicate with the investigators, and manage the studies associated with each set of specimens.
...
Principal investigators and the internal review board now have a structured system to track the state of each request to conductresearch. 
